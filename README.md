...
An AI-powered email assistant that reads your Gmail inbox, categorises messages, suggests quick actions, drafts replies in your writing style, and manages meetings — all from a modern web interface running locally on your machine.

## Features

- **Smart categorisation** — Emails are automatically labelled (important, newsletter, action-required, etc.). Create custom labels and the agent learns from your overrides.
- **Quick actions** — Each email gets context-aware action buttons: reply options, todo tasks, and meeting actions — all generated by a single unified LLM call to prevent redundancy.
- **Style-matched drafting** — A plan → draft → critique → revise → finalise pipeline writes replies that match your personal tone and greeting style.
...
- **Light / dark mode** — Full theme support with proper email body rendering in both modes.
...
Open **http://localhost:8080** in your browser. On first launch it loads emails from the database instantly. Click the refresh button to fetch new emails from Gmail (the first time triggers OAuth sign-in via the browser).
...
agent/
  ...
  decision_suggester.py     # Decision suggestion helpers
